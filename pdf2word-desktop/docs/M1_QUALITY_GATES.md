...
这 6 页验证章节艺术标题、普通正文段落、选项、谈答案/解析/提示、分数与公式可编辑重建、标签去重和页码 `018`。紧凑分数必须输出为原生可编辑 OMML 上下式；“谈+标签”组合标识允许使用源图，但必须作为真实行内对象绑定后续可编辑正文，并核对其数量与宿主关系。题干、选项、答案、解析、提示、分数和公式的正文图片块必须为 0。自动结果只能与同一运行的源页证据对照，不读取历史 OCR、PageModel、DOCX 或人工修补工件。
