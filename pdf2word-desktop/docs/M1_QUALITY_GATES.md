...
这 6 页验证章节艺术标题、普通正文段落、选项、谈答案/解析/提示、分数与公式可编辑重建、标签去重和页码 `018`。题干、选项、答案、解析、提示、分数和公式的正文图片块必须为 0。自动结果只能与同一运行的源页证据对照，不读取历史 OCR、PageModel、DOCX 或人工修补工件。
...
- 章节首页仅排除 Logo、艺术标题、谈字徽标、侧栏、艺术页码和真实图形后，可编辑正文覆盖率不低于 80%；
- 目录文字全部可编辑；
- 公式重页的正文、分数和公式必须可编辑；
