...
桌面端当前只保留与产品主线一致的界面骨架和 `engine_status` 检查。历史固定 12 页 CER 审校界面、文件选择命令和 sidecar 协议已经删除，因为它们依赖过期 PageModel 与人工金标，不属于当前 source-first 端到端链路。

“开始转换”按钮保持禁用，直到 381 页 source-first 管线具备以下条件后再接入：
...
1. 完成并验证 381 页 source-first Python 入口；
