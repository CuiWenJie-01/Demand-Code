...
当前阶段是 M0/M1：可运行的本地转换引擎，包含 PDF 预检、分类、OCR 页渲染基础设施、SQLite 检查点、基础文字 PDF 可编辑导出，以及可供 Tauri 调用的 JSON Lines worker。
...
在目前未安装或尚未接入 OCR 模型的环境中，转换仅支持有可靠文字层的 PDF。对于扫描件和文字转轮廓的 PDF，程序会明确报告需要 OCR 引擎，避免输出伪造的可编辑文本。
