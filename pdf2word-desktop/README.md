...
- `docs/M1_QUALITY_GATES.md`：M1 通用质量门禁；
- `docs/FULL_BOOK_DYNAMIC_QUALITY_GATE.md`：任意页数 PDF 的全量普查、动态抽样、分批执行、严格恢复和最终质量判定的权威实施规范。
- `docs/ADAPTIVE_RULE_ARCHITECTURE.md`：动态识别、源文档配置、候选规则提取、跨文档验证和版本化晋升的架构边界。

当前有效链路从源 PDF 和空任务目录开始，优先使用 GPU 重新渲染、清理水印并 OCR；题干、选项、答案、解析、提示、分数和公式全部写为可编辑 Word 内容，图片只保留 Logo、艺术字、“谈+标签”组合标识、侧栏、艺术页码和真实图形。历史 PageModel/CER/视觉金标及缓存重建入口已经移除，不能再把人工修好的中间结果当作端到端输入。

## 当前状态

- run8 虽通过旧自动门禁，但经用户对照源文档后判定不通过；该结论已作废；
- 物理页 7、8、9、10、21、23 的“正文零图片”当前候选已通过完整自动回归、6 页完整渲染和 Microsoft Word 实机导出逐页目检，并已获得用户确认；当前自动测试共 107 项，后续页面按当前 v2 样式识别处理；
...
- 6 页样式与可编辑性门禁已经完成；不再设置固定页数的全书前置回归批次，下一步严格按照 `docs/FULL_BOOK_DYNAMIC_QUALITY_GATE.md` 实施由预检读取实际页数 `N` 的任意页数 source-first 正式入口、全量普查、严格同任务断点恢复、动态首批页、逐页硬门禁和全书渲染检查；当前大型验收基准的 `N` 为 381；
- 第一批通用抽象已落地：本书页码、目录和人工审校修复已移入严格指纹匹配的 JSON 源配置；新增任意 `N` 页动态抽样选择器与版本化质量阈值。新 PDF 不会继承本书固定页码或修复；
- 任意页数正式 source-first 转换入口尚未实现，因此旧 `convert` 命令和桌面端“开始转换”入口不对外开放；
