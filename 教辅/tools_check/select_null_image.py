...
def main():
    ...
    # -------------------------- 自动推导文件路径 --------------------------
    # 1. 推导平台格式JSONL路径
    jsonl_filename = f"select-{batch}-to平台.json"
    ...
    grandparent_dir = os.path.dirname(os.path.dirname(image_folder))
    log_filename = f"pic_mismatch_log_{batch}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.txt"
    ...
    try:
        ...
        # 4. 命令行输出结果
        print(f"\n" + "="*120)
        print(f"                content['PIC']与物理图片比对结果（{batch}）")
        print("="*120)
        print(f"📊 核心统计：")
