...
function _login($user = null, $password = null)
{
    ...
    if ($user === null and $password === null) {
        ...
        if (isset($_COOKIE['auth'])) {
            ...
            // 判断账号是否存在
            if ($browser_cookie[0] !== 'admin' && !array_key_exists($browser_cookie[0], $guestConfig)) return json_encode(array('code' => 400, 'level' => 0, 'messege' => '账号不存在'));
            // 判断是否管理员（明文验证）
            if ($browser_cookie[0] === 'admin' && $browser_cookie[1] === 'admin@123') return json_encode(array('code' => 200, 'level' => 1, 'messege' => '尊敬的管理员'));
            // 判断是否上传者
